    ...
    <div className="min-h-screen bg-orange-50 text-stone-900">
      <div className="mx-auto max-w-4xl px-6 py-16">
        <button
          ...
          className="mb-10 inline-flex items-center gap-1 rounded-full bg-white px-4 py-2 text-sm font-medium text-stone-500 shadow-sm transition-colors hover:text-orange-600"
        >
          ← Kembali
        </button>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {category.brands.map((brand) => (
            <button
              key={brand.id}
              type="button"
              onClick={() => navigate(`/minuman/${brand.id}`)}
              className="group flex flex-col overflow-hidden rounded-2xl border border-stone-200 bg-white text-left transition-all hover:-translate-y-0.5 hover:border-orange-300 hover:shadow-md hover:shadow-orange-100"
            >
